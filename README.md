...
**Service Worker 策略**:
- HTML: **Network First**（優先網路，失敗才回退快取）
- CSS/JS/JSON: **Cache First**（優先快取）
- 圖片: **Stale While Revalidate**（先回快取、背景更新）
- 第三方請求: **Network Only**（不快取，避免統計/外部資源被快取污染）
...
- ✅ 新增完整 README 開發文件
- ✅ 更新 sitemap：同步更新 sitemap index/page sitemap，並以腳本生成 image sitemap
